...
import { BatchPaymentModal } from "@/components/billing/BatchPaymentModal";
...
export default function BillingModule() {
  ...
  return (
    <Box pos="relative" style={{ height: "calc(100vh - 72px)" }}>
        ...
        <Stack gap="md">
          ...
          <Group justify="space-between" align="center">
            ...
            <Group gap={6}>
              ...
              {/* Consolidated Actions Dropdown (like Demerit module Export UI) */}
              <Menu shadow="md" width={220} position="bottom-end">
                ...
                <Menu.Dropdown>
                  <Menu.Label>Statement of Account</Menu.Label>
                  <Menu.Item
                    leftSection={<IconReceiptTax size={14} />}
                    onClick={() => handleGenerateSoaClick("client")}
                    disabled={!filtered.length}
                  >
                    Generate Client SOA {selectedIds.length > 0 ? `(${selectedIds.length})` : ""}
                  </Menu.Item>
                  <Menu.Item
                    ...
                    disabled={!filtered.length}
                  >
                    Generate Subcon SOA {selectedIds.length > 0 ? `(${selectedIds.length})` : ""}
                  </Menu.Item>
                  ...
                  <Menu.Divider />
                  <Menu.Label>Payment Management</Menu.Label>
                  <Menu.Item
                  ...
                  <Menu.Divider />
                  <Menu.Label>Export & Downloads</Menu.Label>
                  <Menu.Item
          ...
          {/* Summary Cards & Quick Status Tabs */}
          <BillingSummaryCards
            activeFilters={!!activeFilters}
            baseFilteredLength={baseFiltered.length}
            totalRecordsLength={records.length}
            stats={stats}
          ...
          <BillingTable
            ...
            truckCategoryFilter={truckCategoryFilter}
            setTruckCategoryFilter={setTruckCategoryFilter}
            paginated={paginated}
            filteredLength={filtered.length}
            page={page}
            setPage={setPage}
            pageSize={PAGE_SIZE}
            onEditTrip={handleOpenEditTripModal}
            onOpenBillingModal={openBillingModal}
            onDeleteClick={handleDeleteClick}
            onViewPod={setPodPreview}
            getRecordBillStatusKey={getRecordBillStatusKey}
          />
        </Stack>
      </ScrollArea>
      ...
      {/* Batch SOA Payment Modal */}
      <BatchPaymentModal
        opened={batchPaymentModalOpen}
